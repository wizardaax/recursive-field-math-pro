...
//
// Uses opcode maps from mini_assembler with fallback to defaults.

import { wordToTrits, tritsToInt } from './balanced_ternary_arith.js';
import { OP_MAP, MINI_MAP } from './mini_assembler.js';
...
const pairToValue = (s1, s2) => {
  // Convert from balanced ternary pair back to value
  return s1 + s2 * 3;
};
...
export function disasmWord(word, { pc=0 } = {}){
  ...
  const mOp  = pairToValue(T[3], T[4]);
  const mRA  = pairToValue(T[5], T[6]);
  const mArg = pairToValue(T[7], T[8]);
