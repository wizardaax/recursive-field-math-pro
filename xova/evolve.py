...
def evolve(request_path: str):
    ...
    if not cands:
        print(json.dumps(
            {"status": "failed", "errors": [f"No plugin matches api={api} caps={caps}"]},
            indent=2
        ))
        return 1
    ...
    for cand in cands:
        ...
        for link in policy.get("post_chain", []):
            ...
            if not dep:
                errors.append(
                    {"plugin": cand["name"], "warning": f"missing-post:{link['plugin']}"}
                )
                continue
            try:
                ar = run_plugin(
                ...
                errors.append(
                    {"plugin": cand["name"], "error": f"post-exception:{e}"}
                )
