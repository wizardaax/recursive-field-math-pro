  ...
  const env = {
    ...
    async load(text, { useMacros=true } = {}){
      ...
      try {
        ...
        if (useMacros && macroAsm && macroAsm.macroAssembleSync) {
          // Use macro assembler (sync version for immediate results)
          set('Assembling with macros...');
          program = macroAsm.macroAssembleSync(text);
        } else if (miniAsm && miniAsm.assembleProgram) {
          // Fallback to mini assembler
          set('Assembling...');
          program = await miniAsm.assembleProgram(lines);
        } else if (cpuMod && cpuMod.assembleProgram) {
          // Fallback to CPU module assembler
          set('Assembling...');
          program = cpuMod.assembleProgram(lines);
        }
